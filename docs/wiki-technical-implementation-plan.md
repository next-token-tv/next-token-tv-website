...
`tests/visual/wiki-articles.spec.ts` 当前以 DeepSeek 产品／品牌和 DHH 为固定样板，覆盖 390、768、1280、1440、1920、2560 宽度，并用 `glm` 验证无正文回退。若给 GLM 增加正文，必须将回退样例换成实际仍无正文的产品，不能保留失真的断言或删除回退验证。新产品不会自动加入固定样板循环，需要额外逐页检查。
